...
public class Testing {
    ...
    /**
     * Prints a header for a section of tests.  If the
     * header contains multiple lines (i.e. at least one "\n"),
     * then print only enough dashes for the longest line.
     * 
    ...
    {
        if (VERBOSE) {
        	String[] stringParts = sectionTitle.split("\n");
        	int biggest = 0;
        	for (int i=0; i<stringParts.length; i++){
        		if (stringParts[i].length() > biggest){
        			biggest = stringParts[i].length();
        		}
        	}
            int dashCount = biggest;
